import { Cache, APIRunPromptResponse, ErrorResponse, RuntimeOptions, StreamEvents, ApplyTemporaryTokenResult } from "./types"
...
class BaseClient {
  ...
  private static readonly TEMPORARY_TOKEN_HEADER = "X-TEMPORARY-TOKEN"
  ...
  private applyTemporaryToken?: (ctx: { signal?: AbortSignal }) => Promise<ApplyTemporaryTokenResult>;
  ...
  constructor(endpoint: string, token: string, options: RuntimeOptions = {}) {
    ...
    this.applyTemporaryToken = options.applyTemporaryToken
  }

  private ensureTemporaryToken(ctx: { signal?: AbortSignal }): Promise<ApplyTemporaryTokenResult> {
    if (!this.applyTemporaryToken) {
      return Promise.resolve({ token: '', remaining: 0, limit: 0 })
    }
    return this.applyTemporaryToken(ctx)
  }
